...
int Tree::_height(Node *node) {
    if (node == nullptr)  // Caso base
        return 0;
    // verifica qual a maior subarvore e retorna o tamanho dela + 1
    int hleft = _height(node->left);
    int hright = _height(node->right);
    return 1 + (hleft > hright ? hleft : hright);  // Caso geral
}